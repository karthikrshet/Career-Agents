...
const buildGeometry = (width: number, bend: number, thickness: number, pad: number) => {
  ...
  if (a < 0.75) {
    ...
    return {
      ...
      svgH,
      uPerLen: 1,
      point: (u: number, v: number): [number, number] => [u, midY + v],
      angleAt: () => 0,
...
const fmt = (g: any, u: number, v: number) => {
  const [x, y] = g.point(u, v);
  return `${round2(x)} ${round2(y)}`;
};

const edgeSeg = (g: any, uTo: number, v: number, ltr: boolean) => {
...
const bentRectPath = (g: any, u0: number, u1: number, vTop: number, vBot: number, radius: number) => {
...
const bentLinePath = (g: any, u0: number, u1: number, v: number) => `M ${fmt(g, u0, v)} ${edgeSeg(g, u1, v, true)}`;
...
export default function CurvedInput({
  ...
  onChange,
  onSubmit,
  placeholder = "Enter your email for developer updates",
  ...
  const layout = useMemo(() => {
    if (!geom) return null;
    const T = height;
    const btnInset = Math.max(4, borderWidth + 3);
    const chipH = Math.min(30, Math.max(14, T * 0.32));
    const chipW = chipH * 1.25;
    const iconU = 20 + chipW / 2;
    const textStartU = showIcon ? 20 + chipW + 12 : 20;
    const btnW = showButton ? Math.max(btnTextW + fontSize * 2.5, T * 1.25) : 0;
    const btnU1 = geom.W - btnInset;
    const btnU0 = btnU1 - btnW;
    const textEndU = Math.max(textStartU + 20, showButton ? btnU0 - 12 : geom.W - 20);
  ...
  if (geom && layout) {
    ...
    content = (
      ...
      >
        ...
        <path className="curved-input__ring" d={bandPath} fill="none" stroke={accentColor} strokeWidth={borderWidth + 4} />
